...
namespace BE {
	...
	public class UISGReel : MonoBehaviour {
		...
		public void ApplyResult(int[] values )
		{
			...
			//{
			//	// Parse the coordinates string to extract row and column indices
			...
			//	{
			//		int symbolIdx = values[row]; // Assuming row corresponds to reel index
			//		SymbolType symbolType = game.GetSymbol(symbolIdx).type;
			//		 
			//		// Add your animation logic here based on the symbolType and its position
			//		// For example:
			//		//ReelWindow[row, col].Animate(); // Animate symbol at the specified coordinate
			//	}
			//	else
			//	{
			//		// Handle invalid coordinate format
			//		// For example:
			//		Debug.LogError("Invalid coordinate format: " + coordinate);
			//	}
			//}
		}
